<%@ Page Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="ModificarCuentaPagar.aspx.cs" Inherits="YaguareteSystem.UI.Workflow.CuentaPagar.ModificarCuentaPagar" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Datos de Cuentas a pagar</h1>
                </div>
            </div>
        </div>
        <!-- /.container-fluid -->
                                ...
                                <div class="card card-secondary">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-3">
                                                <h5>Nro. Asiento</h5>
                                                <%--onchange="controlExisteNroAsiento(this);"--%>
                                                <asp:TextBox runat="server" ID="txtNroAsiento"  placeholder="Número de asiento" CssClass="form-control"></asp:TextBox>
                                            </div>
                                            <div class="col-lg-3">
                                                <h5>Tipo moneda</h5>
    ...
    <asp:Label runat="server" ID="lblEmpresa" Style="display: none"></asp:Label>
    ...
    <%--<script type="text/javascript">   
        function controlExisteNroAsiento(dato) {
            var valor = parseInt(dato.value);
            var valor2 = dato.value;
            var usu = $('#<%= lblUsuario.ClientID%>').html();
            var pass = $('#<%= lblPass.ClientID%>').html();
            var empresa = $('#<%= lblEmpresa.ClientID%>').html();
            var fechaAddFactura = $('#<%= lblFechaAddFactura.ClientID%>').html();
            debugger;
            console.log(valor);

            if (isNaN(valor)) {
                alert("Ingrese solo números, sin espacios ni comas") 
            } else {
                debugger;
                var settings = {
                    "url": "/../Clases/webServices/ysWebServices.asmx/VerificaSiExisteNroAsientoCuentaPagar?Content-Type=application/json",
                    "method": "POST",
                    "timeout": 0,
                    "headers": {
                ...
                $.ajax(settings).done(function (response) {
                    ...
                    if (response.d.estado) {
                        debugger;
                        alert("Este número de asiento ya se ha cargado.");
                    } 
                });
            }
        }
